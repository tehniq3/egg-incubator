...
ver.2.d - change DS18b20 with LM35 (05.08.2020)
*/
...
#define lmpin A3  // LM35 analog sensor
float vmed = 0;
float ve = 0; 
...
// LM35 part - http://nicuflorica.blogspot.com/2014/01/senzorul-de-temperatura-lm35-si-arduino.html
vmed = 0;
ve=0;
 for (int j = 0; j < 10; j++)  
 {
 int readinglm = analogRead(lmpin);  //getting the voltage reading from the temperature sensor 
 float voltagelm = readinglm * 5.0;  // converting that reading to voltage, for 3.3v arduino use 3.3
 voltagelm /= 1024.0; 
 vmed = vmed + voltagelm;
 delay(20);
 }
 ve = vmed/10; 
 // Serial.print(ve); Serial.println(" volts"); // print out the voltage 
 // now print out the temperature
 float te = ve * 100 ;  //converting from 10 mv per degree 
